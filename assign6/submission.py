...
def create_chain_csp(n):
    ...
    for i in range(n-1):
        csp.add_binary_factor(variables[i], variables[i+1], lambda x, y: 1 if x!=y else 0)
    # END_YOUR_ANSWER
    return csp
...
def create_nqueens_csp(n = 8):
    ...
    column_domain = [i for i in range(1, n+1)] # 열
    variables = ['x%d'%i for i in range(1, n+1)]
    # add variable 
    for i in range(1, n+1):
        domain = [(i, col) for col in column_domain] # 대각 확인을 위한 행 정보도 value에 추가함
        csp.add_variable(variables[i-1], domain)
    # define binary factor
    def binary_factor(val1, val2):
        # 열, 우상향 대각, 우하향 대각 중복 검사
        return (val1[1] != val2[1]) and (val1[0]+val1[1] != val2[0]+val2[1]) and  (val1[0]-val1[1] != val2[0]-val2[1])
    # add binary factor
    for i in range(n):
        for j in range(n):
            if i != j:
                csp.add_binary_factor(variables[i], variables[j], binary_factor)
    # END_YOUR_ANSWER
    return csp
...
class BacktrackingSearch():
    ...
    def get_unassigned_variable(self, assignment):
        ...
        if not self.mcv:
            ...
            MCV = None
            min_possible_counter = float("inf") # MCV가 가질 가능한 선택 개수
            for var in self.csp.variables:
                if var in assignment: 
                    continue
                possible_counter = 0
                for val in self.csp.values[var]:
                    if self.get_delta_weight(assignment, var, val): # consistent한 선택 가능하면 해 개수+1
                        possible_counter += 1
                if possible_counter < min_possible_counter:
                    MCV = var
                    min_possible_counter = possible_counter
            # assert MCV != None
            return MCV
            # END_YOUR_ANSWER
    ...
    def arc_consistency_check(self, var):
        ...
        def enforce_ac3(var1, var2):
            removed_list = []
            for val2 in self.domains[var2]:
                # check unary factor
                if self.csp.unaryFactors[var2] is not None and self.csp.unaryFactors[var2][val2] == 0:
                    removed_list.append(val2)
                    continue
                # check binary factor
                flag = True
                for val1 in self.domains[var1]:
                    # val2에 대해 consistent한 val1이 하나라도 있으면 통과, 없다면 val2 지움
                    if self.csp.binaryFactors[var1][var2] is not None and self.csp.binaryFactors[var1][var2][val1][val2]:
                        flag = False
                if flag:
                    removed_list.append(val2)
                
            if removed_list:
                self.domains[var2] = [val2 for val2 in self.domains[var2] if val2 not in removed_list]
                return True
            else:
                return False
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    csp.add_variable(result, [x for x in range(0, maxSum+1)])

    if len(variables) == 0:
        csp.add_unary_factor(result, lambda val: val==0)
        return result
    # 무지성으로 domain [(0, maxSum), (0, maxSum)]으로 하면 감당안될듯
    # 왜 아래랑 [(cur[1], cur[1]+x) for cur in domain_of_aux for x in csp.values[variables[i]]]랑 다른지?
    domain_of_aux_0 = list({(0, x) for x in csp.values[variables[0]]})
    A_0 = ('aux', name, 0)
    csp.add_variable(A_0, domain_of_aux_0)
    domain_of_aux = domain_of_aux_0
    for i in range(1, len(variables)):
        new_domain_of_aux = list({(cur[1], cur[1]+x) for cur in domain_of_aux for x in csp.values[variables[i]]})
        A_i = ('aux', name, i)
        csp.add_variable(A_i, new_domain_of_aux)
        domain_of_aux = new_domain_of_aux
...
def create_lightbulb_csp(buttonSets, numButtons):
    ...
    for i in range(numButtons): # buttonSets[i] 값이 0이상 정수형이라 가능함
